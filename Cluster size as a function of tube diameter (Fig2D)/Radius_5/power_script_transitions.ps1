...
for ($m = 5 ; $m -le 5 ; $m++){$folder_name =  'Radius_' + $m; mkdir $folder_name; cp KMC_Transitions_0.m $folder_name/KMC_Transitions_0.m ; 
...
for ($num = $start_index ; $num -le $final_index ; $num++){$target_folder = $date + '_' + $num; cd $target_folder; $make_file = $prefix + $num + '.m'; (Get-Content $make_file).replace('par_num = 30', 'par_num = ' + ($start_density_list[$m - 5] + $scaling_factor[$m - 5]*($k - 2)) ) | Set-Content $make_file; cd ..}
