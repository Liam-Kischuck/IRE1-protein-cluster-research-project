...
This file collects several different folders of data into a single graph for Fig. 2D.
The data is spread across different folders because it was hard to get the simulation 
correct at large radii. The data in the other folders were moved into this Additional_Transitions
folder to make it easier to graph the data.

At each radius, the probability of the cluster being wrapped is determined at several different
IRE1 concentrations. Once this probablilties (at roughly 10 different concentration points per radius)
is calculated, the point of 50% wrapping probability is calculated from the linear fit
between the two points that are above and below the 50% wrapping probability. The 'May27'
folder is the data for the first radius, the 'Aug31' folder is the data for the 2nd, 3rd and 4th
radius, the 'Sep4' simulation is for radius 5, and the 'Feb10' simulations in the Additional_Transition
folder is for radius 6 and 7. 
"""
import numpy as np
...
for m in range(1,2):
    ...
    for k in range(1,41):
        for i in range(1,2):
            ...
            for j_index in range(0,25):
                #Determine the amount of time that it is spent wrapped. Then calculate
                #the wrapping probability at this concentration.
    ...
    #The section for finding 0.5 point (the concetration points around which the cluster has
    #a 50% wrapping transition probability)
    ...
    #Calculate the slope between these two points are the 50% wrapping probability to get 
    #the estimated IRE1 concentration at the 50% wrapping probability.
...
plt.xticks(radius_list, [int(ele) for ele in np.round(diameter_list, 0)])
plt.ylabel('Cluster size (proteins)', fontsize = 18)
plt.xlabel('Tube diameter (nm)', fontsize = 18)
plt.legend(frameon = False, fontsize = 12)
plt.savefig('closedsys_wrapping_size_expanded.svg')
plt.show()
