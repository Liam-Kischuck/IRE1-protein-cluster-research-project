...
for ($m = 6 ; $m -le 7 ; $m++){$folder_name =  'Radius_' + $m; mkdir $folder_name; cp KMC_Transitions_0.m $folder_name/KMC_Transitions_0.m ; 
...
for ($num = $start_index ; $num -le $final_index ; $num++){$target_folder = $date + '_' + $num; cd $target_folder; $make_file = $prefix + $num + '.m'; (Get-Content $make_file).replace('par_num = 30', 'par_num = ' + ($start_density_list[$m - 6] + $scaling_factor[$m - 6]*($k - 1)) ) | Set-Content $make_file; cd ..}
for ($num = $start_index ; $num -le $final_index ; $num++){$target_folder = $date + '_' + $num; cd $target_folder; $make_file = $prefix + $num + '.m'; (Get-Content $make_file).replace('Li = 9', 'Li = ' + (3*($m - 1) + 9) ) | Set-Content $make_file; cd ..}
