...
#Only the first 3 radius data were used, since the final raidus would have to run longer to get accurate results.
...
labels = [r'$d_{tube} = 38 nm$',r'$d_{tube} = 48 nm$',r'$d_{tube} = 57 nm$']
date = 'Aug31'

for m in range(1,4):
    #Get the data and graph the plots.
    ...
    k_list = range(1,13)
    if m == 3:
        k_list = range(1,11) #For m = 1 & 2, the point around 50% was too sharp,
                             #so we did extra simulations at another concentration
                             #values, but not for m = 3 (the third radius)
    ...
    for k in k_list: #[1,2,3,4,5...12]
        ...
        for i in range(1,5):
            ...
            print(instance)
            #The k == 11 and 12 for the 1st and 2nd radiuses were done later, so they are in a separte folder.
            if (k != 11) & (k != 12):
            ...
            if len(folder) == 0:  #check if folder is empty
                ...
                for j_index in range(0,6):
                    #Looping over each simulaiton in each file.
                    w_data = y_list[j_index]
                    t_data = x_list[j_index]
                    
                    counting_wrap_time = 0
                    wrapped = False
                    #Determining the total time that the cluster is spent wrapped around tube.
                    if w_data[0] == 1:
    ...
    #The data from the extra k = 11 and 12 simulations have to be inserted into the list at the 
    #appropriate concntration indices.
    if m == 1:
...
plt.xlabel(r"Concentration $(\times 10^2 /\mu m^2)$ ", fontsize = 18)
